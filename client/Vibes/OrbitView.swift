...
struct OrbitView: View {
  ...
  // Widget-mode knobs. Defaults keep main-window behavior exactly as before:
  // sky-only rendering drops the drift dock and the empty-sky invite line
  // (the widget is decoration; invites and drifting friends belong to the
  // restored main window), and a forced-static sky overrides whatever
  // occlusion/reduce-motion would allow so an always-on-screen window costs
  // zero animation work.
  var showsDriftDock = true
  var showsInviteLine = true
  var forcesStatic = false
  ...
  private var allowsMotion: Bool {
    !forcesStatic && windowAllowsAnimation && !reduceMotion
  }
    ...
    VStack(spacing: 0) {
      GeometryReader { geo in
        ...
        .overlay(alignment: .bottom) {
          if showsInviteLine, friends.isEmpty {
      ...
      if showsDriftDock, !drifters.isEmpty {
...
// Sample data lives in SampleFeed.swift (DEBUG-only) so demo mode renders
// exactly what this preview shows. The preview itself is DEBUG-only too —
// #Preview bodies still compile under Release.
#if DEBUG
#Preview("Orbit — four online, three drifting") {
  OrbitView(you: SampleFeed.you, friends: SampleFeed.friends, pulse: SampleFeed.pulse)
